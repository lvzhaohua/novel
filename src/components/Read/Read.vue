<template>
    <section class="read-content">
        ...
        <div class="paging" ref="pagingbar">
          <div @click="padding(-1)">
            <p class="paging-label">
            上一章
            </p>
          </div>
          <div @click="padding(1)">
...
export default {
    ...
    methods: {
        ...
        padding(fb) {
            let i = this.chapterIndex + fb;
            if (i > -1 & i < this.chapters.length) {
              console.log(i);
              this.chapterIndex = i;
              this.chapter = this.chapters[i];
              this.getChapterContent();
              util.setStore('BOOKSOURCE:' + this.$route.query.sourceId, i);
            }
        },
        changeMode() {
